...
function showModalError(message) { if(agentModalError){ agentModalError.textContent = message; agentModalError.style.display = message ? 'block' : 'none'; } }
function clearModalError() { showModalError(''); }
...
async function fetchCategories() {
    if (!db || !collection || !getDocs) { console.error("Firestore functions not available for fetching categories."); return []; }
    ...
    try {
        ...
        snapshot.docs.forEach(doc => { const category = doc.data()?.category; if (category) { categories.add(category.trim()); } });
        availableCategories = Array.from(categories).sort(); 
        console.log("Available categories fetched:", availableCategories);
        populateCategoryCheckboxes();
    } catch (error) { console.error("Error fetching categories:", error); if(categoryPermissionsDiv) categoryPermissionsDiv.innerHTML = '<p style="color: red;">Error loading categories.</p>'; }
}

function populateCategoryCheckboxes() {
    if (!categoryPermissionsDiv) return;
    categoryPermissionsDiv.innerHTML = ''; 
    if (availableCategories.length === 0) { categoryPermissionsDiv.innerHTML = '<p>No categories found or failed to load.</p>'; return; }
    availableCategories.forEach(category => {
        const checkboxId = `category-${category.replace(/\s+/g, '-')}`; 
        const label = document.createElement('label'); label.htmlFor = checkboxId;
        const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.id = checkboxId; checkbox.value = category; checkbox.name = 'allowedCategories';
        label.appendChild(checkbox); label.appendChild(document.createTextNode(` ${escapeHtml(category)}`)); categoryPermissionsDiv.appendChild(label);
    });
}
...
function loadAgents() {
    ...
    if (sortValue === 'name_asc') { firestoreSortField = 'name_lowercase'; firestoreSortDirection = 'asc'; }
    else if (sortValue === 'name_desc') { firestoreSortField = 'name_lowercase'; firestoreSortDirection = 'desc'; }
...
async function handleSaveAgent(event) {
    ...
    try {
        ...
        const agentData = {
            ...
            email: email, contact: contact, status: status,
            canAddCustomers: canAddCustomers, allowedCategories: selectedCategories,
