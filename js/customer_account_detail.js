// js/customer_account_detail.js (Version 2.3.4 - Fixed Table Row Generation & Prep for Scrollbars)
...
async function loadCustomerDetails(customerId) { /* ... (V2.3 जैसा) ... */
    console.log(`V2.3.4: Loading details for customer: ${customerId}`); currentCustomerData = null; if (!db || !doc || !getDoc) { displayError("DB function missing (details)."); return false; }
...
async function loadOrderHistory(customerId) {
    ...
    if (!orderTableBody || !summaryTotalOrdersEl) { console.error("V2.3.4: Order table elements missing."); return; }
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            querySnapshot.forEach(doc => {
                ...
                // --- >>> Using createElement (Fix from V2.3.3) <<< ---
                ...
                cellId.textContent = displayOrderId; // Use textContent
        ...
        console.log(`V2.3.4: Order history loaded.`);
    } catch (error) { console.error("V2.3.4: Error loading order history:", error); displayError(`Error loading orders: ${error.message}`); newOrderTableBody.innerHTML = `<tr><td colspan="4">Error</td></tr>`; summaryTotalOrdersEl.textContent = "Error"; }
}


/** Fetches and displays payment history using createElement. (V2.3.4 - Correct Row Generation) */
async function loadPaymentHistory(customerId) {
    console.log(`V2.3.4: Loading payment history for customer: ${customerId}`);
    const paymentTableBody = document.getElementById('customerPaymentTableBody');
    let totalPaid = 0;

    if (!paymentTableBody) { console.error("V2.3.4: Payment table body missing."); return 0; }
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            querySnapshot.forEach(doc => {
                ...
                const cellDate = row.insertCell();
                cellDate.textContent = paymentDate;

                const cellAmount = row.insertCell();
                cellAmount.textContent = formatCurrency(amountPaid);

                const cellMethod = row.insertCell();
                cellMethod.textContent = escapeHtml(method);

                const cellNotes = row.insertCell();
                cellNotes.textContent = escapeHtml(notes);

                const cellAction = row.insertCell();
                const deleteButton = document.createElement('button');
                deleteButton.className = "button danger-button delete-payment-btn";
                deleteButton.setAttribute('data-payment-id', paymentId);
                deleteButton.title = "Delete Payment";
