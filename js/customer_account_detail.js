// js/customer_account_detail.js (Version 2.3 - Confirmation Modals Implemented & Design Ready)
...
// --- DOM References (Confirmation Modals) ---
...
async function loadCustomerDetails(customerId) { /* ... (V2.2 जैसा) ... */
    console.log(`V2.3: Loading details for customer: ${customerId}`); currentCustomerData = null; if (!db || !doc || !getDoc) { displayError("DB function missing (details)."); return false; }
...
function openConfirmToggleModal(action, newStatus) { /* ... (V2.2 जैसा) ... */ if(!confirmToggleStatusModal||!toggleActionTextSpan||!toggleCustNameSpan||!confirmToggleCheckbox||!confirmToggleBtn||!toggleWarningMessage||!toggleCheckboxLabel||!confirmToggleBtnText){console.error("Toggle status confirmation modal elements missing!");return;}console.log(`V2.3: Opening confirm toggle modal. Action: ${action}, New Status: ${newStatus}`);toggleActionTextSpan.textContent=action;toggleCustNameSpan.textContent=currentCustomerData?.fullName||'this customer';toggleWarningMessage.style.display=(action==='disable')?'block':'none';toggleCheckboxLabel.textContent=`I understand and want to ${action} this account.`;confirmToggleBtnText.textContent=action.charAt(0).toUpperCase()+action.slice(1)+' Account';const icon=confirmToggleBtn.querySelector('i');confirmToggleBtn.className=`button ${action==='disable'?'secondary-button':'success-button'}`;if(icon)icon.className=`fas ${action==='disable'?'fa-toggle-off':'fa-toggle-on'}`;confirmToggleCheckbox.checked=false;confirmToggleBtn.disabled=true;confirmToggleBtn.dataset.newStatus=newStatus;confirmToggleStatusModal.classList.add('active');}
function closeConfirmToggleModal() { /* ... (V2.2 जैसा) ... */ if (confirmToggleStatusModal) confirmToggleStatusModal.classList.remove('active');}
// Event listener for checkbox is set up in setupStaticEventListeners
// Event listener for confirm button is set up in setupStaticEventListeners
async function executeToggleStatus(newStatus) { /* ... (V2.2 जैसा) ... */ if(!updateDoc||!doc||!Timestamp||!currentCustomerId){alert("DB function missing (toggle status)");return;}const button=confirmToggleBtn;button.disabled=true;const originalHTML=button.innerHTML;button.innerHTML=`<i class="fas fa-spinner fa-spin"></i> Processing...`;try{const customerRef=doc(db,"customers",currentCustomerId);await updateDoc(customerRef,{status:newStatus,updatedAt:Timestamp.now()});console.log(`V2.3: Customer status changed to ${newStatus}.`);alert(`Customer account status changed to ${newStatus}.`);closeConfirmToggleModal();await loadCustomerDetails(currentCustomerId);}catch(error){console.error("V2.3: Error toggling status:",error);alert(`Error changing status: ${error.message}`);}finally{button.disabled=false;button.innerHTML=originalHTML;}}
...
function openConfirmDeleteModal() { /* ... (V2.2 जैसा) ... */ if(!confirmDeleteModal||!deleteCustNameSpan||!confirmDeleteCheckboxModal||!confirmDeleteBtn){console.error("Delete confirmation modal elements missing!");return;}console.log("V2.3: Opening delete confirmation modal.");deleteCustNameSpan.textContent=currentCustomerData?.fullName||'this customer';confirmDeleteCheckboxModal.checked=false;confirmDeleteBtn.disabled=true;confirmDeleteModal.classList.add('active');}
function closeConfirmDeleteModal() { /* ... (V2.2 जैसा) ... */ if (confirmDeleteModal) confirmDeleteModal.classList.remove('active');}
 // Event listener for checkbox is set up in setupStaticEventListeners
 // Event listener for confirm button is set up in setupStaticEventListeners
async function executeDeleteCustomer() { /* ... (V2.2 जैसा) ... */ if(!deleteDoc||!doc||!currentCustomerId){alert("DB function missing (delete customer)");return;}const customerName=currentCustomerData?.fullName||`ID: ${currentCustomerId}`;console.log(`V2.3: Executing delete for customer ${currentCustomerId}`);const button=confirmDeleteBtn;button.disabled=true;const originalHTML=button.innerHTML;button.innerHTML=`<i class="fas fa-spinner fa-spin"></i> Deleting...`;try{await deleteDoc(doc(db,"customers",currentCustomerId));console.log("V2.3: Customer deleted.");alert(`Customer "${customerName}" deleted.`);closeConfirmDeleteModal();window.location.href='customer_management.html';}catch(error){console.error("V2.3: Error deleting customer:",error);alert(`Error deleting customer: ${error.message}`);button.disabled=false;button.innerHTML=originalHTML;}}

/** Sets up ALL static event listeners */
function setupStaticEventListeners() {
     console.log("V2.3: Setting up static event listeners...");
     // Order Modal
     ...
     // Add Payment Modal
     if (addPaymentBtn) addPaymentBtn.addEventListener('click', openAddPaymentModal); // Main button opens modal
     ...
     if (addPaymentForm) addPaymentForm.addEventListener('submit', handleSavePayment); // Form submission

     // Edit Customer Modal
     if (editCustomerBtn) editCustomerBtn.addEventListener('click', openEditCustomerModal_CustPage); // Main button opens modal
     ...
     if (customerEditForm) customerEditForm.addEventListener('submit', handleUpdateCustomer_CustPage); // Form submission
     if (creditEditYesRadio && creditEditNoRadio) { // Credit limit show/hide logic
        const toggleCreditLimit = () => { if(creditLimitEditGroup) creditLimitEditGroup.style.display = creditEditYesRadio.checked ? 'block' : 'none'; };
        creditEditYesRadio.addEventListener('change', toggleCreditLimit);
        creditEditNoRadio.addEventListener('change', toggleCreditLimit);
     }

     // Main Page Buttons (triggering confirmation modals)
     ...
     // Confirmation Modals (Toggle Status)
     ...
     // Confirmation Modals (Delete Customer)
     ...
     console.log("V2.3: Static listeners attached.");
}

/** Main function to initialize the page */
window.initializeCustomerDetailPage = async function(user) { /* ... (V2.2 जैसा) ... */
    console.log("V2.3: Initializing customer detail page..."); currentCustomerId = getCustomerIdFromUrl(); if (!currentCustomerId) { displayError("Customer ID missing."); return; } console.log(`V2.3: Customer ID: ${currentCustomerId}`); setupStaticEventListeners(); const customerLoaded = await loadCustomerDetails(currentCustomerId); if (!customerLoaded) return; await loadOrderHistory(currentCustomerId); await loadPaymentHistory(currentCustomerId); updateAccountSummary(); console.log("V2.3: Page initialized successfully.");
}

console.log("customer_account_detail.js (V2.3) loaded.");