// js/new_order.js - v2.2.0 (Adds Calculation Preview + Previous fixes)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!orderForm || !addItemBtn || !orderItemsTableBody || !itemRowTemplate || !calculationPreviewArea || !calculationPreviewContent) { // Check new elements
    ...
    // Event Listeners (same as v2.1.1)
    ...
    if (customerNameInput) { customerNameInput.addEventListener('input', (e) => handleCustomerInput(e, 'name')); customerNameInput.addEventListener('blur', () => setTimeout(() => { if(customerSuggestionsNameBox && !customerSuggestionsNameBox.matches(':hover')) hideSuggestionBox(customerSuggestionsNameBox); }, 150)); }
    if (customerWhatsAppInput) { customerWhatsAppInput.addEventListener('input', (e) => handleCustomerInput(e, 'whatsapp')); customerWhatsAppInput.addEventListener('blur', () => setTimeout(() => { if(customerSuggestionsWhatsAppBox && !customerSuggestionsWhatsAppBox.matches(':hover')) hideSuggestionBox(customerSuggestionsWhatsAppBox); }, 150)); }
...
function handleItemTableClick(event) { /* ... Same as v2.1.1 ... */ if (event.target.closest('.delete-item-btn')) { const r=event.target.closest('.item-row'); if(r){r.remove();hideProductSuggestions();updateOrderSummary();}}}
...
function handleItemTableInput(event) { /* ... Same as v2.1.1 ... */ const t=event.target,r=t.closest('.item-row'); if(!r)return; if(t.matches('.product-name')){activeProductInput=t;handleProductSearchInput(event);}else if(t.matches('.quantity-input, .rate-input, .width-input, .height-input')){updateItemAmount(r);}}
function handleItemTableChange(event){ /* ... Same as v2.1.1 ... */ const t=event.target,r=t.closest('.item-row'); if(!r)return; if(t.matches('.unit-type-select'))handleUnitTypeChange(event); else if(t.matches('.dimension-unit-select'))updateItemAmount(r);}
...
function updateCalculationPreview() {
    ...
    itemRows.forEach((row, index) => {
        ...
        if (unitTypeSelect?.value === 'Sq Feet') {
            foundSqFt = true; // Mark that we found at least one Sq Ft item
            ...
            const quantityInput = row.querySelector('.quantity-input'); // Get quantity

            const unit = dimensionUnitSelect ? dimensionUnitSelect.value : 'feet';
            const width = parseFloat(widthInput?.value || 0);
            const height = parseFloat(heightInput?.value || 0);
            let quantity = parseInt(quantityInput?.value || 1); // Parse quantity
             if (isNaN(quantity) || quantity < 1) quantity = 1; // Default quantity
            ...
            if (width > 0 && height > 0) {
                ...
                if (calcResult && parseFloat(calcResult.printSqFt) > 0) {
                    ...
                    entryContent += `&nbsp; Qty: ${quantity}<br>`; // Display quantity
                    entryContent += `&nbsp; Real Area: ${realSqFtNum.toFixed(2)} sq ft/item<br>`;
                    entryContent += `&nbsp; Print Area: ${printSqFtNum.toFixed(2)} sq ft/item<br>`;
