...
// Updated to include temporary CSV import function
...
window.initializeLicPage = function() {
    ...
    console.log('Debugging - CSV Import Button Element:', importCsvBtn); // Check if import button is found
    ...
    // --- TEMPORARY CSV IMPORT LISTENER ---
    if (importCsvBtn && csvFileInput) {
        console.log('Debugging - Attaching listener to Import CSV button');
        importCsvBtn.addEventListener('click', handleCsvImport);
    } else {
        console.error('ERROR: CSV Import Button or File Input not found!');
    }
...
// ... (this function updated in response #17 - keep that version) ...
function renderPolicyTable(policies) {
    ...
    policies.forEach(policy => {
        ...
        const nameCell = row.insertCell(); const nameLink = document.createElement('a'); nameLink.href = "#"; nameLink.textContent = policy.customerName || 'N/A'; nameLink.title = `View details for ${policy.customerName || 'this client'}`; nameLink.style.cursor = 'pointer'; nameLink.onclick = (e) => { e.preventDefault(); showClientDetail(policy.id, policy.customerName); }; nameCell.appendChild(nameLink);
        row.insertCell().textContent = policy.mobileNo || '-'; row.insertCell().textContent = policy.plan || '-'; row.insertCell().textContent = policy.premiumAmount ? `₹ ${Number(policy.premiumAmount).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '-'; row.insertCell().textContent = policy.modeOfPayment || '-';
        let displayDueDateStr = '-'; const storedNextDueDate = policy.nextInstallmentDate?.toDate();
        if (storedNextDueDate && policy.modeOfPayment) {
             ...
             if (safetyCounter >= maxIterations) { displayDueDateStr = 'Calc Limit'; } else if (displayDate) { displayDueDateStr = formatDate(displayDate); } else { displayDueDateStr = 'Calc Error'; }
        } else if (storedNextDueDate) { displayDueDateStr = formatDate(storedNextDueDate); }
        row.insertCell().textContent = displayDueDateStr;
        const statusCell = row.insertCell(); const statusBadge = document.createElement('span'); const statusText = policy.policyStatus || 'Unknown'; statusBadge.className = `status-badge status-${statusText.toLowerCase()}`; statusBadge.textContent = statusText; statusCell.appendChild(statusBadge);
...
async function handleSavePolicy(event) {
    event.preventDefault(); if (!db) { console.error("DB not initialized"); return; }
    const policyId = editPolicyId.value; const isEditing = !!policyId;
    const customerName = document.getElementById('customerName').value.trim(); const policyNumber = document.getElementById('policyNumber').value.trim(); const mobileNo = document.getElementById('mobileNo').value.trim(); const issuanceDateStr = document.getElementById('issuanceDate').value; const modeOfPayment = document.getElementById('modeOfPayment').value; const premiumAmountStr = document.getElementById('premiumAmount').value;
    if (!customerName || !policyNumber || !mobileNo || !issuanceDateStr || !modeOfPayment || !premiumAmountStr) { alert("Please fill in all required fields (*)."); return; }
    if (!/^\d{10}$/.test(mobileNo)) { alert("Please enter a valid 10-digit mobile number."); return; }
    const premiumAmount = parseFloat(premiumAmountStr); if (isNaN(premiumAmount) || premiumAmount <= 0) { alert("Please enter a valid positive premium amount."); return; }
    const issuanceDate = Timestamp.fromDate(new Date(issuanceDateStr + 'T00:00:00Z'));
    const policyData = { customerName: customerName, fatherName: document.getElementById('fatherName').value.trim() || null, mobileNo: mobileNo, dob: document.getElementById('dob').value ? Timestamp.fromDate(new Date(document.getElementById('dob').value + 'T00:00:00Z')) : null, address: document.getElementById('address').value.trim() || null, policyNumber: policyNumber, plan: document.getElementById('plan').value.trim() || null, sumAssured: parseFloat(document.getElementById('sumAssured').value) || 0, policyTerm: document.getElementById('policyTerm').value.trim() || null, issuanceDate: issuanceDate, modeOfPayment: modeOfPayment, premiumAmount: premiumAmount, maturityDate: document.getElementById('maturityDate').value ? Timestamp.fromDate(new Date(document.getElementById('maturityDate').value + 'T00:00:00Z')) : null, policyStatus: policyStatusModal.value || 'Active', nachStatus: nachStatusModal.value || 'No', updatedAt: serverTimestamp() };
    if (!isEditing) {
        policyData.createdAt = serverTimestamp(); try { const firstDueDate = calculateNextDueDate(issuanceDate.toDate(), modeOfPayment); if (firstDueDate) { policyData.nextInstallmentDate = Timestamp.fromDate(firstDueDate); console.log(`Calculated first due date: ${formatDate(firstDueDate)}`); } else { policyData.nextInstallmentDate = null; console.warn("Could not calculate the first due date."); alert("Warning: Could not automatically calculate the first due date based on Issuance Date and Mode."); } } catch (e) { console.error("Error calculating first due date:", e); policyData.nextInstallmentDate = null; alert("Error calculating the first due date. Please check Mode of Payment."); }
    } else { const existingPolicyData = allPoliciesCache.find(p => p.id === policyId); if (existingPolicyData && existingPolicyData.nextInstallmentDate) { policyData.nextInstallmentDate = existingPolicyData.nextInstallmentDate; } }
    console.log("Saving policy data:", policyData);
    const saveButton = document.getElementById('savePolicyBtn'); if(saveButton) { saveButton.disabled = true; saveButton.textContent = 'Saving...'; }
    try {
        if (isEditing) { const policyRef = doc(db, "licCustomers", policyId); await updateDoc(policyRef, policyData); console.log("Policy updated successfully:", policyId); alert("Policy details updated successfully!");
        } else { if (!policyData.nextInstallmentDate && modeOfPayment) { console.warn("Saving policy without a calculated next installment date."); } const policiesRef = collection(db, "licCustomers"); const docRef = await addDoc(policiesRef, policyData); console.log("Policy added successfully with ID:", docRef.id); alert("New policy added successfully!"); }
        closePolicyModal();
    } catch (error) { console.error("Error saving policy:", error); alert(`Failed to save policy: ${error.message}\nCheck console for details.`);
    } finally { if(saveButton) { saveButton.disabled = false; saveButton.innerHTML = '<i class="fas fa-save"></i> Save Policy'; } }
}
...
// Helper function to parse date strings (DD-MM-YYYY) into Date objects
...
function parseDateString(dateStr) {
    ...
    const month = parseInt(parts[2], 10); // Month is 1-based in input
    ...
    // Further validation can be added (e.g., days in month)

    // Create date object (Month is 0-based for Date constructor)
    const dateObj = new Date(Date.UTC(year, month - 1, day));

    // Final check if the created date is valid and matches input
...
async function handleCsvImport() {
    ...
    if (!db || !collection || !addDoc || !Timestamp || !serverTimestamp) {
         alert("Error: Firebase is not correctly initialized.");
         return;
    }
    ...
    reader.onload = async (event) => {
        ...
        if (importStatusEl) importStatusEl.textContent = "Processing CSV data...";
        console.log("CSV Content Read.");

        // --- Define expected headers and their mapping to Firestore fields ---
        // !! IMPORTANT: This MUST match the header row of YOUR cleaned CSV file !!
        const expectedHeaders = [
            ...
            "policyStatus" // Note: nextInstallmentDate is calculated, not read from CSV
        ];
        ...
        // Simple CSV header parsing (assumes comma delimiter, no commas within fields)
        ...
        try {
            expectedHeaders.forEach(expectedHeader => {
                ...
                if (index === -1) {
                    throw new Error(`Missing expected header column: ${expectedHeader}`);
                }
                headerMap[expectedHeader] = index; // Store index for each field
            });
            console.log("Header Map:", headerMap);
        } catch(e) {
             alert(`CSV Header Error: ${e.message}\nPlease ensure your CSV has the correct headers:\n${expectedHeaders.join(', ')}`);
             if (importCsvBtn) importCsvBtn.disabled = false;
        ...
        for (let i = 0; i < totalRows; i++) {
            ...
            if (values.length < expectedHeaders.length) {
                 console.warn(`Skipping row ${i + 1}: Incorrect number of columns (${values.length} found, ${expectedHeaders.length} expected). Line: ${line}`);
                 errorCount++;
                 continue; // Skip row if column count doesn't match headers
             }

            if (importStatusEl) importStatusEl.textContent = `Importing row ${i + 1} of ${totalRows}...`;
            ...
            try {
                ...
                policyData.dob = parseDateString(policyData.dob) ? Timestamp.fromDate(parseDateString(policyData.dob)) : null;
                ...
                policyData.maturityDate = parseDateString(policyData.maturityDate) ? Timestamp.fromDate(parseDateString(policyData.maturityDate)) : null;

                // Numbers (Remove commas, parse)
                policyData.sumAssured = policyData.sumAssured ? parseFloat(policyData.sumAssured.replace(/,/g, '')) : 0;
                policyData.premiumAmount = policyData.premiumAmount ? parseFloat(policyData.premiumAmount.replace(/,/g, '')) : 0;
                if (isNaN(policyData.sumAssured)) policyData.sumAssured = 0; // Default if parsing fails
                if (isNaN(policyData.premiumAmount)) policyData.premiumAmount = 0; // Default if parsing fails

                // Mobile number validation (basic)
                if (policyData.mobileNo && !/^\d{10}$/.test(policyData.mobileNo)) {
                    console.warn(`Row ${i + 1}: Invalid mobile number format "${policyData.mobileNo}". Storing as is.`);
                    // Optionally set to null: policyData.mobileNo = null;
                }

                // Set defaults if empty
                policyData.policyStatus = policyData.policyStatus || 'Active';
                policyData.nachStatus = policyData.nachStatus || 'No';
                ...
                if (issuanceDateObj && policyData.modeOfPayment) {
                    try {
                         ...
                         console.error(`Row ${i + 1}: Error calculating next due date`, calcError);
                     }
                }
                ...
                // --- Add to Firestore ---
                // console.log(`Adding row ${i+1}:`, policyData); // Uncomment for detailed logging
                await addDoc(collection(db, "licCustomers"), policyData);
                successCount++;

                // Optional: Add a small delay to avoid hitting write limits too quickly
                await new Promise(resolve => setTimeout(resolve, 50)); // 50ms delay

            } catch (error) {
                console.error(`Error processing row ${i + 1}: ${line}`, error);
                errorCount++;
            }
        } // End for loop

        // --- Import complete ---
        const message = `Import complete. Success: ${successCount}, Errors: ${errorCount} (Check console for details).`;
        alert(message);
    ...
    // Re-enable button AFTER starting file read
    if (importCsvBtn) importCsvBtn.disabled = false;
