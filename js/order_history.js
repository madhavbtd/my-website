// js/order_history.js
// Version includes:
// - Base functionality (filtering, sorting, displaying orders)
// - Modal 1 (Details/Edit Popup) logic
// - Modal 2 (WhatsApp Reminder) logic
// - Fix for 'comparison is not defined' error
// - Logic to auto-open modal based on URL parameter 'openModalForId'

// Assume Firebase functions are globally available via order_history.html's script block
const {
    db, collection, onSnapshot, query, orderBy, doc, deleteDoc, updateDoc, Timestamp, arrayUnion
} = window;

// --- DOM Elements ---
const orderTableBody = document.getElementById('orderTableBody');
const loadingRow = document.getElementById('loadingMessage'); // Ensure this element exists in HTML if used
...
let orderIdToOpenFromUrl = null; // Store ID from URL
let modalOpenedFromUrl = false; // Flag to prevent multiple opens
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    orderIdToOpenFromUrl = urlParams.get('openModalForId'); // Check for modal open request
    currentStatusFilter = urlParams.get('status'); // Check for status filter

    if (orderIdToOpenFromUrl) {
        console.log(`[DEBUG] Received request to open modal for Firestore ID: ${orderIdToOpenFromUrl}`);
        // Clean the URL parameter *after* processing to avoid issues if script runs slow
        // We will clean it inside attemptOpenModalFromUrl after successful open attempt
    }
    ...
    waitForDbConnection(() => {
        ...
        if(orderTableBody) {
            orderTableBody.addEventListener('click', function(event) {
                ...
                // Find data from cache - ensure cache is populated!
                 const orderData = allOrdersCache ? allOrdersCache.find(o => o.id === firestoreId) : null;
...
function handleSortChange() {
    ...
    if (field && direction) {
        // No need to re-render if selection hasn't changed
...
function handleFilterChange() { applyFiltersAndRender(); } // Trigger re-render on any filter change
...
function clearFilters() {
    ...
    currentStatusFilter = ''; // Reset status filter variable

    // Clean URL without filter parameters if needed
    if (history.replaceState){
        const cleanUrl = window.location.protocol + "//" + window.location.host + window.location.pathname;
        window.history.replaceState({path:cleanUrl},'',cleanUrl);
    }
    applyFiltersAndRender(); // Re-apply filters (which are now empty)
}
...
function listenForOrders() {
    ...
    if (orderTableBody) { orderTableBody.innerHTML = `<tr><td colspan="9" id="loadingMessage">Loading orders...</td></tr>`; } // Colspan=9

    try {
        const q = query(collection(db, "orders")); // Fetch all initially, sorting/filtering done client-side
        ...
        unsubscribeOrders = onSnapshot(q, (snapshot) => {
             ...
             // Process docs, ensure data structure is consistent
            allOrdersCache = snapshot.docs.map(doc => {
                 const data = doc.data();
                 // Basic data validation/defaults can happen here if needed
                 return {
                    id: doc.id, // Firestore document ID
                    orderId: data.orderId || '', // Custom Order ID field
                    customerDetails: data.customerDetails || {},
                    products: data.products || [],
                    orderDate: data.orderDate || null, // Expecting 'YYYY-MM-DD' string
                    deliveryDate: data.deliveryDate || null, // Expecting 'YYYY-MM-DD' string
            ...
            attemptOpenModalFromUrl(); // Attempt to open modal after data is loaded/updated
        }, (error) => {
             console.error("[DEBUG] Error fetching orders snapshot:", error);
             if (orderTableBody) { orderTableBody.innerHTML = `<tr><td colspan="9" style="color: red;">Error loading orders. Please try again.</td></tr>`; }
        });
    } catch (error) {
        console.error("[DEBUG] Error setting up Firestore listener:", error);
        if (orderTableBody) { orderTableBody.innerHTML = `<tr><td colspan="9" style="color: red;">Error setting up listener.</td></tr>`; }
    }
}
...
function applyFiltersAndRender() {
    ...
    let filteredOrders = allOrdersCache.filter(order => {
        ...
        if (filterSearchValue) {
            ...
            if (!(orderIdString.toLowerCase().includes(filterSearchValue) || customerNameString.toLowerCase().includes(filterSearchValue) || firestoreIdString.toLowerCase().includes(filterSearchValue) || productsString.includes(filterSearchValue) || mobileString.includes(filterSearchValue))) return false;
        }
        return true;
    });
    ...
    try {
        filteredOrders.sort((a, b) => {
            ...
            if (currentSortField === 'orderDate' || currentSortField === 'deliveryDate') {
                valA = valA ? new Date(valA).getTime() : null; // Convert to number for comparison
                valB = valB ? new Date(valB).getTime() : null;
                if (valA === null && valB === null) return 0;
                if (valA === null) return currentSortDirection === 'asc' ? 1 : -1; // Nulls last in asc
                if (valB === null) return currentSortDirection === 'asc' ? -1 : 1; // Nulls last in asc
            }

            // *** Use renamed variable 'sortComparison' ***
            let sortComparison = 0;

            // Basic comparison for numbers, strings etc.
            if (valA > valB) {
                sortComparison = 1;
            } else if (valA < valB) {
                sortComparison = -1;
        ...
        // Handle sort error, maybe default to unsorted or log it
    }
    ...
    if (filteredOrders.length === 0) {
        orderTableBody.innerHTML = `<tr><td colspan="9" id="noOrdersMessage">No orders found matching your criteria.</td></tr>`;
    } else {
        filteredOrders.forEach(order => {
            displayOrderRow(order.id, order); // Pass Firestore ID and full data object
        });
    }
}

// --- Function to attempt opening modal based on URL param ---
function attemptOpenModalFromUrl() {
    if (orderIdToOpenFromUrl && allOrdersCache.length > 0 && !modalOpenedFromUrl) {
        ...
        if (orderToOpenData) {
            ...
            modalOpenedFromUrl = true; // Set flag
            // Clean URL parameter now after successful attempt
            // This removes only the specific parameter
             //const currentUrl = new URL(window.location);
             //currentUrl.searchParams.delete('openModalForId');
             //window.history.replaceState({}, '', currentUrl.toString());
             // Or simpler, remove all params:
            ...
            // It might appear in a subsequent snapshot, the flag prevents multiple opens
        }
    }
}
...
function displayOrderRow(firestoreId, data) {
    if (!orderTableBody) return; // Safety check
    const tableRow = document.createElement('tr');
    tableRow.setAttribute('data-id', firestoreId); // Use Firestore ID as data-id
    ...
    // Ensure date strings are handled correctly, provide fallback
    ...
    const displayId = data.orderId || `(Sys: ${firestoreId.substring(0,6)}...)`; // Show custom ID or fallback
    const status = data.status || 'Unknown';
    const priority = data.urgent === 'Yes' ? 'Yes' : 'No'; // Normalize priority

    // Format products safely
    let productsHtml = '-';
    if (Array.isArray(data.products) && data.products.length > 0) {
        productsHtml = data.products.map(p => `${String(p.name||'Unnamed').replace(/</g,"&lt;")} (${p.quantity||'?'})`).join('<br>');
    }
    ...
    tableRow.innerHTML = `
        <td>${displayId}</td>
        <td>
            <span class="customer-name-display">${customerName.replace(/</g,"&lt;")}</span>
            <span class="customer-mobile-inline">${customerMobile.replace(/</g,"&lt;")}</span>
        </td>
        <td>${productsHtml}</td>
        <td>${orderDateStr}</td>
...
// --- Modal 1 Handling (Details/Edit Popup) ---
...
async function handleUpdateStatus() {
    ...
    try {
        ...
        await updateDoc(orderRef, {
            status: newStatus,
            updatedAt: historyEntry.timestamp, // Update the main timestamp
            statusHistory: arrayUnion(historyEntry) // Add new history entry atomically
        });
        console.log("[DEBUG] Status updated successfully for:", firestoreId);
        closeDetailsModal(); // Close modal immediately
...
async function handleDeleteOrder(firestoreId, orderDisplayId) {
    ...
    if (confirm(`Are you sure you want to delete Order ID: ${orderDisplayId}? This cannot be undone.`)) {
        try {
            console.log("[DEBUG] Deleting order:", firestoreId);
        ...
        } catch (error) { console.error("Error deleting order:", error); alert("Error deleting order: " + error.message); }
...
function getWhatsAppMessageTemplate(status, customerName, orderId, deliveryDate) {
     ...
     switch (status) {
        ...
        case "Ready for Working": // <-- TEMPLATE CHANGED FOR THIS STATUS
     ...
     // ******** END OF MESSAGE TEMPLATE UPDATE ********

     // Replace placeholders using regex for global replacement
     ...
     message += `\n\n${signature}`; // Signature is appended here
     return message;
}
