...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let orderIdForModal = null;
    let orderDisplayIdForModal = null;
    let customerDetailsForModal = null;
    ...
    for (const key in elements) {
        if (!elements[key]) {
             if (key.startsWith('whatsapp') || key.startsWith('popupCloseBtnHistory') || key.startsWith('modalOrderInfo') || key.startsWith('modalProductList') || key.startsWith('modalRemarksText') ) {
                 console.warn(`DEBUG: Optional element reference missing or null for "${key}".`);
             } else {
            ...
            ordersForPage.forEach((order, index) => {
                ...
                if (whatsappNo) {
                    ...
                    if (cleanWhatsAppNo) {
                        ...
                        whatsappCell.innerHTML = `<a href="https://wa.me/<span class="math-inline">\{cleanWhatsAppNo\}?text\=</span>{message}" target="_blank" title="Send WhatsApp to ${customerName}" class="whatsapp-icon"><i class="fab fa-whatsapp"></i></a>`;
    ...
    async function openDetailsModal(orderDocId, displayId) {
        ...
        try {
            ...
            if (docSnap.exists()) {
                ...
                // Populate only if elements exist
                ...
                // Clear other fields on error too
    ...
    async function saveStatusFromModal() {
        ...
        const currentOrderData = allOrders.find(o => o.id === orderIdForModal); // Use allOrders for comparison
        const originalStatus = currentOrderData?.status;

        if(newStatus === originalStatus){ alert("Status has not changed."); return; }
        console.log(`DEBUG: Attempting update: <span class="math-inline">\{orderIdForModal\} from '</span>{originalStatus}' to '${newStatus}'`);
    ...
    function triggerWhatsAppPopupHistory(customer, orderId, status) {
        console.log("DEBUG: triggerWhatsAppPopupHistory called for status:", status);
        if (!elements.whatsappReminderPopupHistory || !elements.whatsappMsgPreviewHistory || !elements.whatsappSendLinkHistory) {
            console.error("DEBUG: WhatsApp popup elements missing in history page."); return;
        }
        ...
        if (!customerNumber) { console.warn("DEBUG: WhatsApp No missing for popup."); return; }
        let message = '';
        const signature = "\n\n*Madhav Multy Print*\n9549116541";
        switch (status) {
            case 'Verification': message = `नमस्ते *<span class="math-inline">\{customerName\}\*,\\n\\nआपके ऑर्डर \(ID\: \*</span>{orderId}*) का डिज़ाइन वेरिफिकेशन के लिए तैयार है।\nकृपया डिज़ाइन को ध्यानपूर्वक चेक करें। *OK* का जवाब देने के बाद कोई बदलाव संभव नहीं होगा。\n\nधन्यवाद!`; break;
            case 'Ready for Working': message = `नमस्ते *<span class="math-inline">\{customerName\}\*,\\n\\nआपका ऑर्डर \(ID\: \*</span>{orderId}*) अब प्रिंटिंग/वर्किंग के लिए तैयार है。\n\nधन्यवाद!`; break;
            case 'Delivered': message = `नमस्ते *<span class="math-inline">\{customerName\}\*,\\n\\nआपका ऑर्डर \(ID\: \*</span>{orderId}*) सफलतापूर्वक डिलीवर कर दिया गया है。\n\nहमें सेवा का अवसर देने के लिए धन्यवाद!`; break;
            default: return;
        }
        message += signature;
        ...
        elements.whatsappReminderPopupHistory.classList.add('active');
        console.log("DEBUG: WhatsApp reminder shown from history page.");
    }
    ...
    function closeWhatsAppPopupHistory() {
        if (elements.whatsappReminderPopupHistory) {
            ...
            console.log("DEBUG: History WhatsApp popup closed.");
        }
    }
    ...
    if (elements.modalSaveStatusBtn) elements.modalSaveStatusBtn.addEventListener('click', saveStatusFromModal);
    ...
    const checkDbInterval = setInterval(() => {
        ...
        if (window.db && typeof window.onSnapshot === 'function' && typeof window.updateDoc === 'function') {
            ...
            // console.log("DEBUG: Waiting for DB connection and functions..."); // Can be verbose
        }
    }, 100); // Check faster
