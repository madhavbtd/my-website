...
function clearModalError() {
    showModalError('');
    clearFieldErrors(); // Also clear field-specific errors
}
...
function openAgentModal(mode = 'add', agentData = null) {
    ...
    if (authUidInput) authUidInput.value = '';
    ...
    if (mode === 'add') {
        ...
        if (agentData.allowedCategories && Array.isArray(agentData.allowedCategories)) {
            agentData.allowedCategories.forEach(categoryName => {
                try {
                    const checkbox = categoryPermissionsDiv.querySelector(`input[value="${categoryName}"]`);
                    if (checkbox) {
                        checkbox.checked = true;
                    }
                } catch (e) {
...
async function handleSaveAgent(event) {
    ...
    if (!db || !doc || !setDoc || !updateDoc || !collection || !serverTimestamp) {
    ...
    try {
        ...
        const authUid = authUidInput?.value.trim();  // Get authUid from input
        ...
        // Validate individual fields and display errors
        let isValid = true;
        if (!name) {
        ...
        if (agentPasswordInput.required && (!password || password.length < 6)) {
            showFieldError('password', 'Password is required and must be at least 6 characters.');
            isValid = false;
        }
        ...
        const agentData = {
            authUid: authUid,
            name: name,
            name_lowercase: name.toLowerCase(), // For sorting/searching
            email: email,
            contact: contact,
            status: status,
            userType: userType,
        ...
        showModalError("Error saving agent: " + error.message);
    } finally {
        saveAgentBtn.disabled = false;
        if (saveAgentBtnText) saveAgentBtnText.textContent = originalButtonText;
    }
