// new_order.js
// Version: Combined updates + Delay Timer for Initialization + Balance Display Fix

// --- Delay Timer ---
// Wait for a short period before trying to access Firebase functions
// This is a workaround for potential timing issues with module script loading
...
setTimeout(() => {
    ...
    // Ensure DOM is ready *before* accessing elements (should be fine inside setTimeout after DOMContentLoaded)
    ...
    const customerCurrentBalanceSpan = document.getElementById('customerCurrentBalance'); // This is the <strong> tag
    ...
    function initializeAppLogic() {
        console.log("New Order DOM Loaded (Combined Updates + Delay). Initializing App Logic...");
    ...
    // =========================================================================
    // ===          MODIFIED fetchAndDisplayCustomerDetails Function         ===
    // =========================================================================
    async function fetchAndDisplayCustomerDetails(customerId) {
        ...
        try {
            ...
            // --- Balance Calculation and Display Logic ---
            let balanceText = 'Calculating...';
            let balanceNum = NaN;
            let initialBalanceFetchError = false; // Flag for calculation errors

            // Display 'Calculating...' initially
            if(customerBalanceArea && customerCurrentBalanceSpan){
                 customerCurrentBalanceSpan.textContent = balanceText;
                 customerCurrentBalanceSpan.classList.remove('positive-balance', 'negative-balance', 'zero-balance'); // Clear classes
            ...
            try {
                ...
                if (totalOrderValue !== 'N/A') { // Only calculate if order total is valid
                    balanceNum = Number(totalOrderValue) - Number(totalPaid);
                } else if (totalPaid > 0) { // If only payments exist, balance is negative (credit)
                     balanceNum = -totalPaid;
                     balanceText = `(Paid: ${formatCurrency(totalPaid)})`; // Show paid amount if orders couldn't load
                ...
                balanceText = 'Error';
                balanceNum = NaN;
                initialBalanceFetchError = true; // Set error flag
            }

            // --- ** NEW DISPLAY LOGIC START ** ---
            if(customerBalanceArea && customerCurrentBalanceSpan){
                let displayValue = 'Error';
                let balanceClass = ''; // No default class
                let titleText = `Balance: ${balanceText}`; // Default title

                // Remove previous classes
                customerCurrentBalanceSpan.classList.remove('positive-balance', 'negative-balance', 'zero-balance');

                if (!isNaN(balanceNum)) { // Only proceed if balance calculation was successful
                    const tolerance = 0.01; // Threshold for considering zero

                    if (balanceNum > tolerance) { // Customer Owes (Due) - Positive number
                        balanceClass = 'negative-balance'; // Class for Red color
                        // Add minus sign manually for display consistency with customer management page
                        displayValue = "-" + formatCurrency(balanceNum);
                        titleText = `Due: ${formatCurrency(balanceNum)}`;
                    } else if (balanceNum < -tolerance) { // Customer has Credit - Negative number
                        balanceClass = 'positive-balance'; // Class for Green color
                        // Use Math.abs() to remove the minus sign for display
                        displayValue = formatCurrency(Math.abs(balanceNum));
                        titleText = `Credit: ${formatCurrency(Math.abs(balanceNum))}`;
                    } else { // Zero Balance
                        balanceClass = 'zero-balance'; // Class for Grey color (Ensure CSS exists)
                        displayValue = formatCurrency(0);
                        titleText = 'Zero Balance';
                    }
                    customerCurrentBalanceSpan.textContent = displayValue; // Set the formatted text
                    if (balanceClass) {
                        customerCurrentBalanceSpan.classList.add(balanceClass); // Add the correct class
                    }
                } else {
                     // If balanceNum is NaN (due to calculation error), show the error text
                     customerCurrentBalanceSpan.textContent = balanceText;
                }
                customerCurrentBalanceSpan.title = titleText; // Set the tooltip
            ...
            // Pass the calculated balance number (or NaN if error) to checkCreditLimit
            checkCreditLimit(initialBalanceFetchError ? NaN : balanceNum);
    ...
    // =========================================================================
    // ===            End of MODIFIED fetchAndDisplayCustomerDetails          ===
    // =========================================================================
        ...
        try {
            const nextIdNum = await runTransaction(db, async (transaction) => {
                ...
                if (counterDoc.exists() && counterDoc.data().lastId) {
                    ...
                    else { console.warn(`Counter '${counterName}' lastId is not a number (${counterDoc.data().lastId}). Resetting to startId.`); nextId = startId; }
                } else { console.log(`Counter '${counterName}' not found or lastId missing, starting at ${startId}.`); }
                transaction.set(counterRef, { lastId: nextId }, { merge: true });
                return nextId;
            });
            return prefix ? `${prefix}${nextIdNum}` : nextIdNum;
        } catch (error) { console.error(`Error getting next ID for ${counterName}:`, error); throw new Error(`Failed to generate ID for ${counterName}. Error: ${error.message}`); }
    }

    async function handleFormSubmit(event) {
        event.preventDefault();
        console.log("Submit initiated (Combined Updates)...");
        showFormError('');
        ...
        try {
            ...
            if (isEditMode) { oId = currentOrderData?.orderId || eId || orderIdToEdit; if (!oId) throw new Error("Internal Error: Missing Order ID for update."); console.log(`Using existing Order ID for update: ${oId}`);
                ...
                catch (idError) { console.error("Error generating Order ID:", idError); showFormError(`Failed to generate automatic Order ID: ${idError.message}`); throw idError; /* Let outer catch handle button reset */ }
            }
            ...
            const payload = { orderId: oId, customerId: cId, customerDetails: cD, orderDate: Timestamp.fromDate(new Date(oDV + 'T00:00:00')), deliveryDate: dDV ? Timestamp.fromDate(new Date(dDV + 'T00:00:00')) : null, urgent: uV, remarks: rV, status: sS, items: items, subTotal: subT, discountPercentage: dP || 0, discountAmount: cDA, totalAmount: finalAmount, finalAmount: finalAmount, updatedAt: serverTimestamp() };
            let savedId, msg;

            if (isEditMode) { if (!orderIdToEdit) throw new Error("Internal Error: Missing Firestore Document ID for update."); delete payload.createdAt; if (currentOrderData && sS !== currentOrderData.status) { const historyUpdate = { status: sS, timestamp: serverTimestamp() }; payload.statusHistory = arrayUnion(historyUpdate); } else { delete payload.statusHistory; } payload.orderSource = currentOrderData?.orderSource || 'Manual'; await updateDoc(doc(db, "orders", orderIdToEdit), payload); savedId = orderIdToEdit; msg = `Order ${oId} updated successfully!`;
            } else { payload.createdAt = serverTimestamp(); payload.statusHistory = [{ status: sS, timestamp: Timestamp.now() }]; payload.orderSource = 'Manual'; const orderDocRef = await addDoc(collection(db, "orders"), payload); savedId = orderDocRef.id; msg = `Order ${oId} created successfully!`; if(displayOrderIdInput) displayOrderIdInput.value = oId; }
            console.log(msg, "Firestore Doc ID:", savedId);

            if (aP > 0 && !isEditMode) { console.log(`Advance payment amount entered: ${aP}. Creating payment record...`); try { const paymentData = { customerId: cId, orderRefId: savedId, orderId: oId, amountPaid: aP, paymentDate: serverTimestamp(), paymentMethod: "Order Advance", notes: `Advance payment for Order #${oId}`, createdAt: serverTimestamp() }; const paymentDocRef = await addDoc(collection(db, "payments"), paymentData); console.log(`Advance payment record added successfully. Payment Doc ID: ${paymentDocRef.id}`); } catch (paymentError) { console.error("Error saving advance payment record:", paymentError); alert(`Order ${oId} was saved successfully, but there was an error recording the advance payment: ${paymentError.message}\n\nPlease add the payment manually later.`); } }
    ...
    console.log("new_order.js script logic initialized (Combined Updates + Delay + Balance Fix).");
