// js/new_po.js - v17 (Option A Rate Edit + Fetch Purchase Price on Order Load)
...
// (remains unchanged)
function getOrCreateProductSuggestionsDiv() { if (!productSuggestionsDiv) { productSuggestionsDiv = document.createElement('div'); productSuggestionsDiv.className = 'product-suggestions-list'; productSuggestionsDiv.style.display = 'none'; document.body.appendChild(productSuggestionsDiv); } return productSuggestionsDiv; }
function positionProductSuggestions(inputElement) { const suggestionsDiv = getOrCreateProductSuggestionsDiv(); const rect = inputElement.getBoundingClientRect(); suggestionsDiv.style.left = `${rect.left + window.scrollX}px`; suggestionsDiv.style.top = `${rect.bottom + window.scrollY}px`; suggestionsDiv.style.width = `${rect.width}px`; suggestionsDiv.style.display = 'block'; }
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            querySnapshot.forEach((docSnapshot) => {
                ...
                div.dataset.rate = product.purchasePrice ?? '';
                div.dataset.unitFirestore = product.unit || 'Qty';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const hasSourceOrder = urlParamsInit.has('sourceOrderId');
    if (!isEditingInit && !hasSourceOrder && poOrderDateInput && !poOrderDateInput.value) { try { poOrderDateInput.value = new Date().toISOString().split('T')[0]; } catch (e) { console.error("Error setting default date:", e); } }
    if (addItemBtn && itemRowTemplate && poItemsTableBody) { addItemBtn.addEventListener('click', () => { const templateContent = itemRowTemplate.content.cloneNode(true); poItemsTableBody.appendChild(templateContent); const appendedRow = poItemsTableBody.lastElementChild; if (appendedRow?.matches('.item-row')) { addItemRowEventListeners(appendedRow); const firstInput = appendedRow.querySelector('.product-name'); if(firstInput) firstInput.focus(); } else { console.error("Failed to get appended row or it's not an item-row"); } }); } else { console.error("Add Item button, Item Row template or Table Body not found!"); }
    if (supplierSearchInput && supplierSuggestionsDiv && selectedSupplierIdInput && selectedSupplierNameInput) { supplierSearchInput.addEventListener('input', handleSupplierSearchInput); document.addEventListener('click', (e) => { if (supplierSuggestionsDiv && supplierSuggestionsDiv.style.display === 'block' && !supplierSearchInput.contains(e.target) && !supplierSuggestionsDiv.contains(e.target)) { supplierSuggestionsDiv.style.display = 'none'; } if (productSuggestionsDiv && productSuggestionsDiv.style.display === 'block' && activeProductInput && !activeProductInput.contains(e.target) && !productSuggestionsDiv.contains(e.target)) { hideProductSuggestions(); } }); } else { console.warn("Supplier search elements not found."); }
    ...
    if (poItemsTableBody) { poItemsTableBody.addEventListener('input', handleProductSearchInput); poItemsTableBody.addEventListener('focusin', (event) => { if (event.target.matches('.product-name')) { activeProductInput = event.target; } }); } else { console.error("Cannot add product search listener: Table body not found."); }
    ...
    else if (sourceOrderId && urlItemIndicesParam) { if (urlSupplierId && urlSupplierName && supplierSearchInput && selectedSupplierIdInput && selectedSupplierNameInput) { console.log("Pre-filling supplier from URL:", urlSupplierName, urlSupplierId); supplierSearchInput.value = urlSupplierName; selectedSupplierIdInput.value = urlSupplierId; selectedSupplierNameInput.value = urlSupplierName; supplierSearchInput.readOnly = true; supplierSearchInput.style.backgroundColor = '#e9ecef'; } const itemIndices = urlItemIndicesParam.split(',').map(Number).filter(i => !isNaN(i)); console.log("Loading items from source order:", sourceOrderId, "Indices:", itemIndices); loadOrderDataForPO(sourceOrderId, itemIndices); if (poNumberInput) poNumberInput.value = ''; if (poOrderDateInput && !poOrderDateInput.value) { try { poOrderDateInput.value = new Date().toISOString().split('T')[0]; } catch(e){} } }
    else { if (poItemsTableBody.children.length === 0 && addItemBtn) { addItemBtn.click(); } if (poNumberInput) poNumberInput.value = ''; updateTotalAmount(); updateFullCalculationPreview(); }
    console.log("new_po.js v17 (Rate Fix): Setup and listeners added."); // Version bump
});

// --- Supplier Search Implementation ---
// (remains unchanged)
function handleSupplierSearchInput() { if (!supplierSearchInput || !supplierSuggestionsDiv || !selectedSupplierIdInput || !selectedSupplierNameInput) return; clearTimeout(supplierSearchDebounceTimer); const searchTerm = supplierSearchInput.value.trim(); selectedSupplierIdInput.value = ''; selectedSupplierNameInput.value = ''; if (searchTerm.length < 1) { supplierSuggestionsDiv.innerHTML = ''; supplierSuggestionsDiv.style.display = 'none'; return; } supplierSearchDebounceTimer = setTimeout(() => { fetchSupplierSuggestions(searchTerm); }, 350); }
async function fetchSupplierSuggestions(searchTerm) { if (!supplierSuggestionsDiv) return; supplierSuggestionsDiv.innerHTML = '<div>Loading...</div>'; supplierSuggestionsDiv.style.display = 'block'; const searchTermLower = searchTerm.toLowerCase(); try { console.log(`Searching suppliers with lowercase term: "${searchTermLower}"`); const q = query( collection(db, "suppliers"), orderBy("name_lowercase"), where("name_lowercase", ">=", searchTermLower), where("name_lowercase", "<=", searchTermLower + '\uf8ff'), limit(10) ); const querySnapshot = await getDocs(q); supplierSuggestionsDiv.innerHTML = ''; if (querySnapshot.empty) { console.log(`No suppliers found matching lowercase: "${searchTermLower}"`); supplierSuggestionsDiv.innerHTML = '<div class="no-suggestions">No matching suppliers found. <button type="button" id="suggestAddNewSupplier" class="button link-button" style="margin-left: 10px;">+ Add New</button></div>'; const suggestBtn = supplierSuggestionsDiv.querySelector('#suggestAddNewSupplier'); if (suggestBtn && addNewSupplierFromPOBtn) { suggestBtn.addEventListener('click', () => addNewSupplierFromPOBtn.click()); } } else { querySnapshot.forEach((docSnapshot) => { const supplier = docSnapshot.data(); if (supplier.name_lowercase === undefined) { console.warn(`Supplier ${docSnapshot.id} (${supplier.name}) is missing 'name_lowercase' field.`); } const supplierId = docSnapshot.id; const div = document.createElement('div'); div.textContent = `${supplier.name}${supplier.companyName ? ' (' + supplier.companyName + ')' : ''}`; div.dataset.id = supplierId; div.dataset.name = supplier.name; div.style.cursor = 'pointer'; div.addEventListener('mousedown', (e) => { e.preventDefault(); supplierSearchInput.value = supplier.name; if(selectedSupplierIdInput) selectedSupplierIdInput.value = supplierId; if(selectedSupplierNameInput) selectedSupplierNameInput.value = supplier.name; supplierSuggestionsDiv.style.display = 'none'; if (poOrderDateInput) poOrderDateInput.focus(); }); supplierSuggestionsDiv.appendChild(div); }); } } catch (error) { console.error("Error fetching supplier suggestions:", error); if (error.message.includes("indexes are required")) { console.error("Firestore Index Missing: Please create a Firestore index for the 'suppliers' collection on the 'name_lowercase' field (ascending)."); supplierSuggestionsDiv.innerHTML = '<div class="no-suggestions" style="color:red;">Search Error (Index Missing).</div>'; } else if (error.message.includes("does not support order by")) { console.error("Firestore Field Missing or Index Error: Ensure 'name_lowercase' field exists and is indexed."); supplierSuggestionsDiv.innerHTML = '<div class="no-suggestions" style="color:red;">Search Error (Field/Index).</div>'; } else { supplierSuggestionsDiv.innerHTML = '<div class="no-suggestions" style="color:red;">Error fetching suppliers.</div>'; } } }


// ========================================================================
// <<<--- UPDATED Load Order Data for PO Function ---<<<
// ========================================================================
        ...
        if (orderDocSnap.exists()) {
            ...
            const rowPromises = itemIndices.map(async (index) => {
                 if (index >= 0 && index < sourceItems.length) {
                    const orderItem = sourceItems[index]; // Item from the sales order
                    if (orderItem && orderItem.productName) {
                        console.log(`Processing item from order index ${index}:`, orderItem);

                        // <<<--- START: Fetch Purchase Price ---<<<
                        let purchasePrice = ''; // Default to empty
                            ...
                            try {
                                ...
                                if (productSnap.exists()) {
                                    ...
                                    if (productData.purchasePrice !== undefined && productData.purchasePrice !== null) {
                                        ...
                                        console.log(`Fetched purchase price ${purchasePrice} for product ${orderItem.productId}`);
                                    } else {
                                        console.warn(`Purchase price not found in product master for ${orderItem.productId}`);
                                    }
                                } else {
                                    console.warn(`Product master not found for ID ${orderItem.productId}`);
                                }
                            } catch (prodError) {
                                console.error(`Error fetching product master for ${orderItem.productId}:`, prodError);
                            }
                        } else {
                             console.warn(`productId missing in order item index ${index}, cannot fetch purchase price.`);
                        }
                        // <<<--- END: Fetch Purchase Price ---<<<

                        // Create a new object for populateItemRow, using purchasePrice for rate
                        const itemDataForPO = {
                            ...orderItem, // Copy other details from order item
                            rate: purchasePrice // <<<--- Use fetched purchasePrice instead of orderItem.rate
                        };

                        const templateContent = itemRowTemplate.content.cloneNode(true);
                        const newRow = templateContent.querySelector('.item-row');
                        if (newRow) {
                            // Populate using itemDataForPO which has the purchase price
                            await populateItemRow(newRow, itemDataForPO); // populateItemRow remains the same (uses itemData.rate)
                     ...
                     console.warn(`Item index ${index} is out of bounds for source items.`);
                }
            });

            await Promise.all(rowPromises); // Wait for all rows to be populated
            ...
            updateFullCalculationPreview(); // Update calculations
...
// ========================================================================
// --- End UPDATED Load Order Data for PO ---
// ========================================================================


// ========================================================================
// <<<--- populateItemRow function (Option A: Use Saved PO Rate) ---<<<
// This is the version from v16, which correctly handles Option A for editing
// It uses itemData.rate, which is now correctly supplied by loadOrderDataForPO
// ========================================================================
async function populateItemRow(rowElement, itemData) {
    ...
    // --- Populate dimensions (same as before) ---
    ...
    // --- Set Rate and Unit Type ---
    let finalRate = '';
    let finalUnitType = 'Qty'; // Default unit type

    // *** OPTION A LOGIC: Prioritize the rate passed in itemData ***
    // (This now works for both edit (using saved PO rate) and load-from-order (using fetched purchase price))
    if (itemData.rate !== undefined && itemData.rate !== null) {
        finalRate = String(itemData.rate);
        console.log(`Using passed rate: ${finalRate} for item: ${itemData.productName}`);
    } else {
        console.warn(`Rate not found in itemData for: ${itemData.productName}`);
        finalRate = '';
    }
    // *** END OPTION A LOGIC ***

    // Determine Unit Type
    const itemUnitInfo = itemData.unitType || 'Qty';
    ...
    // Set the rate input using the determined finalRate
    if (rateInput) {
        rateInput.value = finalRate;
    }

    // Set the unit type dropdown (same as before)
    if (unitTypeSelect) {
    ...
    // Focus rate input after population if it's empty
...
console.log("new_po.js v17 (Rate Fix) loaded and ready."); // Version bump