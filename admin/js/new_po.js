// js/new_po.js - v16 (Option A: Prioritize Saved PO Rate on Edit)
...
// handleUnitTypeChange, addItemRowEventListeners - remain unchanged from previous version (v15)
...
// getOrCreateProductSuggestionsDiv, positionProductSuggestions, hideProductSuggestions,
// handleProductSearchInput, fetchProductSuggestions, selectProductSuggestion - remain unchanged
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            querySnapshot.forEach((docSnapshot) => {
                ...
                div.dataset.rate = product.purchasePrice ?? ''; // Store purchasePrice here
                div.dataset.unitFirestore = product.unit || 'Qty';

                div.addEventListener('mousedown', (e) => {
                    e.preventDefault();
                    selectProductSuggestion(product, inputElement); // Pass full product data
                });
...
function selectProductSuggestion(productData, inputElement) {
    ...
    // --- Rate Setting based on Product Selection ---
    // Even though we prioritize saved rate on EDIT, when selecting a product
    // from suggestion, it makes sense to populate the CURRENT purchase price
    // as a suggestion for the user. They can override it.
    // If editing, the populateItemRow function will later override this based on Option A.
...
// (remains unchanged from previous version v15)
document.addEventListener('DOMContentLoaded', () => {
    console.log("new_po.js v16 (Option A Rate): DOM loaded."); // Version bump
    ...
    // Initialize Product Search Listener
    if (poItemsTableBody) {
    ...
    if (editPOIdValue) {
        ...
        if (urlSupplierId && urlSupplierName && supplierSearchInput && selectedSupplierIdInput && selectedSupplierNameInput) {
            ...
            supplierSearchInput.readOnly = true;
            supplierSearchInput.style.backgroundColor = '#e9ecef';
        }
        ...
        loadOrderDataForPO(sourceOrderId, itemIndices); // Call the new function

        if (poNumberInput) poNumberInput.value = ''; // Ensure PO number is clear for new PO
        // Set order date here if not already set by loadOrderDataForPO (optional)
        if (poOrderDateInput && !poOrderDateInput.value) {
             try { poOrderDateInput.value = new Date().toISOString().split('T')[0]; } catch(e){}
        }
         // Initial calculation/update might be needed after items load
         // Let loadOrderDataForPO handle the initial calculation calls

    } else {
        // --- Completely New PO Mode ---
        if (poItemsTableBody.children.length === 0 && addItemBtn) {
            // Add the first empty row only if no items were loaded from order
            addItemBtn.click();
        }
    ...
    console.log("new_po.js v16 (Option A Rate): Setup and listeners added."); // Version bump
});
...
async function loadOrderDataForPO(orderId, itemIndices) {
    ...
    try {
        ...
        if (orderDocSnap.exists()) {
            ...
            const rowPromises = itemIndices.map(async (index) => {
                 if (index >= 0 && index < sourceItems.length) {
                    ...
                    if (item && item.productName) {
                        ...
                        if (newRow) {
                            // Populate the row asynchronously (includes product fetch)
                            await populateItemRow(newRow, item); // <<<--- Make this async
            ...
            // If no items were added (e.g., bad indices), ensure there's at least one row
...
// ========================================================================
// <<<--- UPDATED populateItemRow function (Option A: Use Saved PO Rate) ---<<<
// ========================================================================
async function populateItemRow(rowElement, itemData) {
    ...
    // *** OPTION A Change: Prioritize the rate saved with the PO item ***
    if (itemData.rate !== undefined && itemData.rate !== null) {
        finalRate = String(itemData.rate); // Use the rate from the PO item data
        console.log(`Using saved PO rate: ${finalRate} for item: ${itemData.productName}`);
    } else {
        // Optional: If rate is not found in PO data, maybe then try fetching product?
        // For now, we leave it blank if not in itemData, as per Option A preference.
        console.warn(`Rate not found in saved PO item data for: ${itemData.productName}`);
        finalRate = ''; // Or set to 0? Decide based on preference.
    }
    // *** END OPTION A Change ***

    // Determine Unit Type (can still use fetched product data for this if needed, or just from itemData)
    const itemUnitInfo = itemData.unitType || 'Qty'; // Use unitType saved with the item
    ...
    // --- REMOVED/COMMENTED OUT: Fetching product data to get the latest rate ---
    /*
    let fetchedProductData = null;
    if (itemData.productId) {
        // ... (code to fetch product data using productId) ...
        // Previous logic used fetchedProductData.purchasePrice here
    }
    */
    // --- END REMOVED/COMMENTED OUT ---

    // Set the rate input using the determined finalRate (from itemData.rate)
    if (rateInput) {
...
// ========================================================================
// --- End UPDATED populateItemRow (Option A) ---
// ========================================================================
    ...
    savePOBtn.disabled = true; if (savePOBtnSpan) savePOBtnSpan.textContent = isEditing ? 'Updating...' : 'Saving...'; const poData = { supplierId: supplierId, supplierName: supplierName, poNumber: finalPoNumber, orderDate: Timestamp.fromDate(new Date(orderDateValue + 'T00:00:00')), items: itemsArray, totalAmount: parseFloat(calculatedTotalAmount.toFixed(2)), notes: notes, updatedAt: serverTimestamp() }; if (!isEditing) { poData.createdAt = serverTimestamp(); poData.status = 'New'; } else { poData.status = editingPOData?.status || 'Updated'; } console.log("DEBUG PO Gen: Final PO Data being sent to Firestore:", poData); try { let successMessage = ''; let poDocId = editingPOId; // Use existing ID if editing
     if (isEditing) { const poDocRef = doc(db, "purchaseOrders", editingPOId); await updateDoc(poDocRef, poData); successMessage = `Purchase Order ${poData.poNumber} updated successfully!`; console.log(successMessage, "ID:", editingPOId); } else { if (typeof poData.poNumber !== 'number' || isNaN(poData.poNumber) || poData.poNumber <= 0) { throw new Error(`Invalid PO number detected just before save: ${poData.poNumber}`); } const poDocRef = await addDoc(collection(db, "purchaseOrders"), poData); poDocId = poDocRef.id; // Get the new ID
      ...
      if (sourceOrderIdForLink && poDocId && poData.poNumber) {
          ...
          try {
              await updateDoc(orderRef, {
                  linkedPOs: window.arrayUnion ? window.arrayUnion({ // Use arrayUnion if available globally
                       poId: poDocId,
                       poNumber: poData.poNumber,
                       supplierName: poData.supplierName,
                       createdAt: poData.createdAt || Timestamp.now() // Use created timestamp or now
                  }) : ( (await getDoc(orderRef)).data()?.linkedPOs || [] ).concat([{ // Fallback with read first
                       poId: poDocId,
                       poNumber: poData.poNumber,
                       supplierName: poData.supplierName,
                       createdAt: poData.createdAt || Timestamp.now()
                  }])
              });
              ...
              // Don't stop the process, just log the error. Maybe show a non-critical warning.
              showPOError("PO Saved, but error occurred linking it back to the original order.");
          }
      }
      // --- End Link PO ---
      ...
      if (sourceOrderIdForLink) {
           // If came from an order, go back to order history, potentially opening that order's details
           window.location.href = `order_history.html?openModalForId=${sourceOrderIdForLink}`; // Redirect back and try to open modal
      } else {
          window.location.href = 'supplier_management.html'; // Default redirect
      }
...
console.log("new_po.js v16 (Option A Rate) loaded and ready."); // Version bump