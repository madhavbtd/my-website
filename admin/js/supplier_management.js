// js/supplier_management.js - v29 + Balance Calculation
...
// --- DOM Elements ---
// (ये आपकी मूल फ़ाइल के अनुसार ही रहेंगे)
...
const supplierContactInput = document.getElementById('supplierContactInput'); // Make sure this exists in your HTML/JS
...
let cachedPOs = {};
// let currentPoQuery = null; // Assume original file didn't use these pagination states extensively
...
// --- START: Added Code for Balance Calculation ---
...
async function getSupplierAdjustmentTotals(supplierId) {
    ...
    try {
        ...
        if (error.code === 'failed-precondition') {
            console.warn(`Firestore index missing for supplierAccountAdjustments by supplierId (${supplierId}). Balance might be inaccurate.`);
        }
        return { totalDebit: 0, totalCredit: 0 };
    }
}

// *** REPLACED Function: Calculate and Display Supplier Balance ***
async function calculateAndDisplaySupplierBalance(supplierId, balanceCellElement) {
    ...
    balanceCellElement.style.textAlign = 'right'; // Ensure right alignment

    try {
        // Fetch data concurrently using the helper functions
        ...
        // Display formatted balance and add CSS class for coloring
        balanceCellElement.classList.remove('balance-due', 'balance-credit', 'balance-zero'); // Remove old classes

        if (finalBalance > 0.005) { // You owe (Payable/Due)
            balanceCellElement.textContent = formatCurrency(finalBalance);
            balanceCellElement.classList.add('balance-due'); // Class for RED color
        } else if (finalBalance < -0.005) { // You have Credit
            balanceCellElement.textContent = formatCurrency(Math.abs(finalBalance)) + " Cr"; // Add Cr suffix
            balanceCellElement.classList.add('balance-credit'); // Class for GREEN color
        } else { // Zero balance
            balanceCellElement.textContent = formatCurrency(0);
             balanceCellElement.classList.add('balance-zero'); // Class for normal/gray color
        }

    } catch (error) {
        console.error(`Error calculating balance for supplier ${supplierId}:`, error);
        balanceCellElement.textContent = 'Error';
        balanceCellElement.classList.add('balance-due'); // Use red color to indicate error
    }
}
// --- END: Added Code for Balance Calculation ---
...
async function displaySupplierTable() {
    ...
    try {
        ...
        if (querySnapshot.empty) {
            ...
            const balancePromises = []; // Keep track of balance calculation promises
            querySnapshot.forEach((docSnapshot) => {
                ...
                const contact = escapeHtml(supplier.whatsappNo || supplier.contact || '-'); // Use 'contact' field if available

                // --- Make sure this innerHTML does NOT contain the stray comment ---
                tr.innerHTML = `
                    <td>${name}</td>
                    <td>${contact}</td>
                    <td class="supplier-balance" style="text-align: right;">Loading...</td>
                ...
                // --- Ensure this call exists and targets the right element ---
                balancePromises.push(calculateAndDisplaySupplierBalance(supplierId, tr.querySelector('.supplier-balance')));
            });
            // Wait for all balance calculations to attempt completion
...
// --- All other functions from your original supplier_management.js below this line ---
// (populateSupplierFilterDropdown, handleSupplierTableActions, handleSupplierRowClick,
//  openSupplierModal, closeSupplierModal, handleAddSupplierSubmit, deleteSupplier,
//  handleDeleteSupplierFromModal, displayPoList (KEEP YOUR ORIGINAL VERSION), handlePoSort,
//  updateSortIndicators, handlePOTableActions, markPOAsReceived, openStatusModal,
//  closeStatusModal, handleStatusUpdate, openPoShareModal, closePoShareModalFunction,
//  handleCopyPoShareContent, handlePrintPoShare, openSeeMoreItemsModal, closePoItemsModal,
//  updateBatchActionBar, Batch Action Listeners, initializeSupplierManagementPage,
//  setupEventListeners, window.initializeSupplierManagementPage)

// --- Make sure the rest of your original code follows here ---
// Example: (You need to copy the rest of your functions here)
...
function handleSupplierRowClick(event) {
    const row = event.target.closest('tr'); if (event.target.closest('button')) { return; }
    const supplierId = row?.dataset.id; if (supplierId) { window.location.href = `supplier_account_detail.html?id=${supplierId}`; }
}

// Supplier Modal/CRUD Functions (Assume your original versions)
...
async function handleAddSupplierSubmit(event) {
    ...
    if (!db || !collection || !addDoc || !doc || !updateDoc || !serverTimestamp || !Timestamp || !supplierContactInput) { showSupplierFormError("Error: Database functions missing."); return; }
    ...
    const supplierData = {
        ...
        contact: supplierContactInput.value.trim(), // Make sure contact is included
...
// --- PO List Functions (KEEP YOUR ORIGINAL FILTER LOGIC) ---
async function displayPoList() {
    // --- This is your original PO display/filter logic ---
    // --- Make sure this section matches your previously working version ---
    ...
    try {
        ...
        const statusFilter = poStatusFilter.value; // YOUR ORIGINAL HANDLING OF THIS
        const startDateVal = poStartDateFilter.value;
        const endDateVal = poEndDateFilter.value;

        // --- YOUR ORIGINAL FILTER CONDITIONS ---
        if (supplierFilterId) { conditions.push(where("supplierId", "==", supplierFilterId)); }
        // *** Keep YOUR original logic for statusFilter here ***
        // Example (assuming you only filtered main status):
        if (statusFilter) {
             console.log(`Applying original status filter: status == ${statusFilter}`)
             conditions.push(where("status", "==", statusFilter));
        }
        // *** End of Section to Keep Original ***
        ...
        // --- Sorting (Keep this updated version) ---
        const sortFieldMapping = { poNumber: 'poNumber', orderDate: 'orderDate', supplierName: 'supplierName', totalAmount: 'totalAmount', status: 'status', paymentStatus: 'paymentStatus', createdAt: 'createdAt'};
        ...
        const finalQuery = query(baseQuery, ...conditions, ...sortClauses); // Keep using finalQuery
        const querySnapshot = await getDocs(finalQuery); // Keep using finalQuery
        ...
        // --- Render Loop (Keep updated version with status helpers) ---
        filteredDocs.forEach(docRef_po => {
            ...
            let statusClass = getStatusClass(statusText); // Use helper
            let paymentStatusText = po.paymentStatus || 'Pending';
            let paymentStatusClass = getPaymentStatusClass(paymentStatusText); // Use helper
            ...
            if (po.items && po.items.length > 0) {
                ...
                if (po.items.length === 1) { const qty = escapeHtml(firstItem.quantity || '?'); let sizeText = ''; if (firstItem.unitType === 'Sq Feet' && firstItem.width && firstItem.height) { const width = escapeHtml(firstItem.width); const height = escapeHtml(firstItem.height); const unit = escapeHtml(firstItem.dimensionUnit || 'units'); sizeText = ` [${width} x ${height} ${unit}]`; } itemsHtml = `<span class="product-name-display">${firstItemName}</span> (Qty: ${qty})${sizeText}`; }
        ...
        if (error.code === 'failed-precondition') { showPoListError(`Error: Firestore index missing. Check console.`); } else { showPoListError(`Error loading POs: ${error.message}`); }
...
async function initializeSupplierManagementPage(user) {
    ...
    try {
        await displaySupplierTable(); // Load suppliers first
        await displayPoList(); // Then load POs
...
function setupEventListeners() {
    ...
    // --- PO List Related ---
    if (poFilterBtn) {
        poFilterBtn.addEventListener('click', () => { displayPoList(); });
    }
    if (poClearFilterBtn) {
    ...
    if (poSearchInput) {
        poSearchInput.addEventListener('keypress', (event) => {
            if (event.key === 'Enter') { event.preventDefault(); if (poFilterBtn) poFilterBtn.click(); else displayPoList(); }
        });
    }
    ...
    if(poTableBody) { poTableBody.addEventListener('change', (event) => { if (event.target.classList.contains('po-select-checkbox')) { const checkbox = event.target; const poId = checkbox.value; const row = checkbox.closest('tr'); if (checkbox.checked) { selectedPoIds.add(poId); if(row) row.classList.add('selected-row'); } else { selectedPoIds.delete(poId); if(row) row.classList.remove('selected-row'); } updateBatchActionBar(); } }); }
    if(deselectAllPoBtn) { deselectAllPoBtn.addEventListener('click', () => { selectedPoIds.clear(); poTableBody.querySelectorAll('.po-select-checkbox').forEach(checkbox => checkbox.checked = false); poTableBody.querySelectorAll('tr.selected-row').forEach(row => row.classList.remove('selected-row')); if (selectAllPoCheckbox) selectAllPoCheckbox.checked = false; updateBatchActionBar(); }); }
    if(batchApplyStatusBtn && batchUpdateStatusSelect) { batchApplyStatusBtn.addEventListener('click', async () => { /* ... (batch apply status logic) ... */ const newStatus = batchUpdateStatusSelect.value; const idsToUpdate = Array.from(selectedPoIds); const MAX_LIMIT = 10; if (!newStatus) { alert("Please select a status to apply."); return; } if (idsToUpdate.length === 0) { alert("Please select at least one PO."); return; } if (idsToUpdate.length > MAX_LIMIT) { alert(`You can update status for a maximum of ${MAX_LIMIT} POs at a time.`); return; } if (confirm(`Apply status '${escapeHtml(newStatus)}' to ${idsToUpdate.length} selected PO(s)?`)) { batchApplyStatusBtn.disabled = true; batchApplyStatusBtn.textContent = 'Applying...'; let successCount = 0; let errorCount = 0; const batch = writeBatch(db); idsToUpdate.forEach(poId => { const poRef = doc(db, "purchaseOrders", poId); batch.update(poRef, { status: newStatus, updatedAt: serverTimestamp() }); }); try { await batch.commit(); successCount = idsToUpdate.length; } catch (error) { console.error(`Error committing batch status update:`, error); errorCount = idsToUpdate.length; successCount = 0; alert(`Error applying batch status update: ${error.message}`); } batchApplyStatusBtn.disabled = false; batchApplyStatusBtn.textContent = 'Apply Status'; if(successCount > 0) alert(`Batch update complete. Status updated for ${successCount} PO(s).`); selectedPoIds.clear(); poTableBody.querySelectorAll('.po-select-checkbox').forEach(cb => cb.checked = false); poTableBody.querySelectorAll('tr.selected-row').forEach(row => row.classList.remove('selected-row')); updateBatchActionBar(); displayPoList(); } }); }
    if(batchMarkReceivedBtn) { batchMarkReceivedBtn.addEventListener('click', async () => { /* ... (batch mark received logic) ... */ const idsToUpdate = Array.from(selectedPoIds); if (idsToUpdate.length === 0) { alert("Please select at least one PO."); return; } if (confirm(`Mark ${idsToUpdate.length} selected PO(s) as 'Product Received'?`)) { batchMarkReceivedBtn.disabled = true; batchMarkReceivedBtn.textContent = 'Marking...'; let successCount = 0; let errorCount = 0; const batch = writeBatch(db); idsToUpdate.forEach(poId => { const poRef = doc(db, "purchaseOrders", poId); batch.update(poRef, { status: 'Product Received', receivedDate: serverTimestamp(), updatedAt: serverTimestamp() }); }); try { await batch.commit(); successCount = idsToUpdate.length; } catch (error) { console.error("Error batch marking received:", error); errorCount = idsToUpdate.length; alert(`Error: ${error.message}`); } batchMarkReceivedBtn.disabled = false; batchMarkReceivedBtn.textContent = 'Mark as Received'; if(successCount > 0) alert(`Mark Received complete. Updated ${successCount} PO(s).`); selectedPoIds.clear(); updateBatchActionBar(); displayPoList(); } }); }
    if(batchDeletePoBtn) { batchDeletePoBtn.addEventListener('click', async () => { /* ... (batch delete logic) ... */ const idsToDelete = Array.from(selectedPoIds); const MAX_LIMIT = 5; if (idsToDelete.length === 0) { alert("Please select at least one PO to delete."); return; } if (idsToDelete.length > MAX_LIMIT) { alert(`You can delete a maximum of ${MAX_LIMIT} POs at a time.`); return; } if (confirm(`Permanently delete ${idsToDelete.length} selected PO(s)? This cannot be undone.`)) { batchDeletePoBtn.disabled = true; batchDeletePoBtn.textContent = 'Deleting...'; let successCount = 0; let errorCount = 0; const batch = writeBatch(db); idsToDelete.forEach(poId => { batch.delete(doc(db, "purchaseOrders", poId)); }); try { await batch.commit(); successCount = idsToDelete.length; } catch (error) { console.error(`Error committing batch delete:`, error); errorCount = idsToDelete.length; alert(`Error deleting POs: ${error.message}`); } batchDeletePoBtn.disabled = false; batchDeletePoBtn.textContent = 'Delete Selected'; if(successCount > 0) alert(`Batch delete complete. Deleted ${successCount} PO(s).`); selectedPoIds.clear(); poTableBody.querySelectorAll('.po-select-checkbox').forEach(cb => cb.checked = false); poTableBody.querySelectorAll('tr.selected-row').forEach(row => row.classList.remove('selected-row')); updateBatchActionBar(); displayPoList(); } }); }
...
// --- Assuming Auth listener calls initializeSupplierManagementPage ---
// Example: onAuthStateChanged(auth, (user) => { if (user) { initializeSupplierManagementPage(user); } else { /* Redirect to login */ } });